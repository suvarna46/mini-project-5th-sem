...
{
    char arr1[8],arr2[8],arr3[8],name[8],mnemonic[8],b[8];
    int i=0,j,k,locctr=0000,startaddr,r,temp,t,finaladdr,found=0,opcode,ch=0;
    FILE *fp1,*fp2,*fp3;
    fp1=fopen(file,"r");
    ...
    printf("\n Press 1 to see the output after pass 1 ");
    fflush(stdin);
    scanf("%d",&ch);
    if(ch==1)
    {
...
{
    FILE *fp4,*fp5,*fp6,*fp;
    char arr4[8],arr5[8],arr6[8],buff[50],counter[8];
    int i=0,j,k,c,t=0,size=0,start,final,ch=0;
   ...
   fp4=fopen("intermediate.txt","r");
    ...
    fprintf(fp5,"T^00%s^%d",counter,size);
    fclose(fp5);
    while(strcmp(arr4,"END")!=0)
     {
    ...
    printf("\n press 2 to see the output after pass 2 ");
    fflush(stdin);
    scanf("%d",&ch);
    if(ch==2)
    {
